...
public class BasicMinecartRenderer<T extends AbstractMinecart> extends MinecartRenderer<T> {

    private final boolean showMinecart;

    public BasicMinecartRenderer(EntityRendererProvider.Context context, ModelLayerLocation cartLocation, boolean showMinecart) {
        super(context, cartLocation);
        this.showMinecart = showMinecart;
    }

    @Override
    @ParametersAreNonnullByDefault
    public void render(T pEntity, float pEntityYaw, float pPartialTicks, PoseStack pMatrixStack, MultiBufferSource pBuffer, int pPackedLight) {
        RenderNameplateEvent renderNameplateEvent = new RenderNameplateEvent(pEntity, pEntity.getDisplayName(), this,
                pMatrixStack, pBuffer, pPackedLight, pPartialTicks);
        MinecraftForge.EVENT_BUS.post(renderNameplateEvent);
        if (renderNameplateEvent.getResult() != Result.DENY && (renderNameplateEvent.getResult() == Result.ALLOW ||
                this.shouldShowName(pEntity))) {
        ...
        this.renderMinecartContents(pEntity, pPartialTicks, pEntity.getDisplayBlockState(), pMatrixStack, pBuffer, pPackedLight);
